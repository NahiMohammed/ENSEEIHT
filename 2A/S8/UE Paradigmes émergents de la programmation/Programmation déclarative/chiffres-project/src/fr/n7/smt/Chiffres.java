...
public class Chiffres {
    ...
    private BoolExpr pushNumFormula(int step, int num) {
        ...
        // Variables représentant la pile et l'index à l'étape actuelle et à l'étape suivante
        ...
        BoolExpr stackUpdate = context.mkEq(nextIdx, context.mkAdd(idx, context.mkInt(1)));
	    BoolExpr idxUpdate = context.mkEq(nextStack, context.mkStore(stack, idx, context.mkBV(num, bvBits)));
        
        res = context.mkAnd(stackUpdate, idxUpdate);

        for (int j = 0; j < step; j++) {
            res = context.mkAnd(res, context.mkNot(pushNumVar(j, num)));
        }
    ...
    private BoolExpr transitionFormula(int step) {
        ...
        // Combiner les formules de transition en une seule formule
        BoolExpr combinedTransitions = context.mkOr(transitionFormulas.toArray(new BoolExpr[0]));
    
        // Retourner la formule finale qui lie les états aux pas step et step + 1
        return context.mkAnd(exactlyOneAction, combinedTransitions);
    }
     ...
     */
        ...
        BitVecExpr topValue = (BitVecExpr) context.mkSelect(stack, addressTop);
    
        // Vérifie que l'index de la pile est égal à 1 (la pile contient exactement un élément)
        BoolExpr idxIsOne = context.mkEq(idx, context.mkInt(1));
    
        // Vérifie que la valeur au sommet de la pile est égale à la valeur cible
        BoolExpr topIsTarget = context.mkEq(topValue, toBvNum(this.target));
    
        return context.mkAnd(idxIsOne, topIsTarget);
