...
0
        m(s) =f(xk) + transpose(gradf(xk))*s + (1/2)*transpose(s)*hessf(xk)*s
        
        rho_k = (f(xk)-f(xk+sk))/(m(zeros(length(x0)))-m(sk))
        ...
        nb_iters+=1
        xs = vcat(xs, [xk])
        ### Verification des conditions d'arret ###
        if norm(gradf(xk)) <= max(tol_rel*norm(gradf(x0)),tol_abs )
            flag = 0
            break
        elseif  norm(sk) <= epsilon*max(tol_abs, tol_rel*norm(xk))
            flag = 1
            break
        elseif abs(f(xk+sk ) - f(xk)) <= epsilon*max(tol_rel*abs(f(xk)),tol_abs) 
            flag = 2
            break
