    ...
    static class PartialMax extends RecursiveTask<Integer> {
        ...
        protected Integer compute() {
            if (end - start > threshold) {
                // Diviser le travail en deux sous-tâches
                int middle = (start + end) / 2;
                PartialMax leftTask = new PartialMax(array, start, middle, threshold);
                PartialMax rightTask = new PartialMax(array, middle, end, threshold);
                leftTask.fork();
                rightTask.fork(); // Ajout de la ligne manquante
                int rightResult = rightTask.join();
                int leftResult = leftTask.join();
                return Math.max(leftResult, rightResult);
            } else {
                // Cas de base : utiliser directement LargeIntArray.max
