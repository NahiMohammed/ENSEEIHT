...
let appartient mot (Trie(arbre, decompose, recompose)) = 
  let mot_decompose=(decompose mot) in
  appartient_arbre mot_decompose arbre ;; 
...
let trie_sujet =
  List.fold_right ajout
    ...
    let%test _ = not (appartient "o" trie_sujet)
...
let tester_retrait = retrait "de" trie_sujet
let%test _ = (appartient "de"  trie_sujet)
let%test _ =  not (appartient "de"  tester_retrait)
