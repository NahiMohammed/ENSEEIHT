...
(*   paramètres : - une liste d'éléments (caractères dans le cas d'un dico)   *)
(*                - un arbre n-aire                                           *)
(*   résultat   : l'arbre n-aire sans le mot supprimé                         *)
(******************************************************************************)
...
(*****************************************************************************************************************)
(*   fonction qui renvoie la liste des éléments dans l’ordre lexicographique d’un arbre passé en paramètre       *)
(*   signature  : lister_arbre : 'a list -> 'a arbre -> 'a arbre                                                *)
(*   paramètres : - un arbre n-aire                                                                              *)
(*   résultat   : la liste des éléments dans l’ordre lexicographique                                             *)
(*****************************************************************************************************************)


let rec lister_arbre (Noeud (b, lb)) =
   match b, lb with
   | true, [] -> [[]]
   | false, [] -> []
   | _, _ ->
     let mot_branchees = List.flatten (List.map (fun (a, ar) -> List.map (fun list -> a :: list) (lister_arbre ar)) lb) in
     let mots_noeuds = List.map (fun (_, ar) -> lister_arbre ar) lb in
     mot_branchees @ List.flatten mots_noeuds
...
let rec lister_arbre (Noeud (b, lb)) =
   ...
   | _, _ ->
     let mot_branchees = List.flatten (List.map (fun (a, ar) -> List.map (fun list -> a :: list) (lister_arbre ar)) lb) in
     let mots_noeuds = ... in
     mot_branchees @  mots_noeuds
 
 
let result = lister_arbre arbre_sujet
...
module Arbre = struct
   ...
   (* Fonction récursive pour éliminer les branches inutiles *)
   let rec eliminer_branches_inutiles (arbre : 'a arbre) : 'a arbre =
     match arbre with
     | Noeud (true, branches) ->
       (* Si le nœud est marqué comme vrai, on conserve les branches *)
       let branches' = List.map (fun (valeur, sous_arbre) -> (valeur, eliminer_branches_inutiles sous_arbre)) branches in
       Noeud (true, branches')
     | Noeud (false, []) ->
       (* Si le nœud est marqué comme faux et n'a pas de branches, on le supprime *)
       Noeud (false, [])
     | Noeud (false, branches) ->
       (* Si le nœud est marqué comme faux mais a des branches, on supprime les branches inutiles *)
       let branches' = List.map (fun (valeur, sous_arbre) -> (valeur, eliminer_branches_inutiles sous_arbre)) branches in
       let branches' = List.filter (fun (_, branche) -> not (est_inutile branche)) branches' in
       if branches' = [] then
         Noeud (false, [])
       else
         Noeud (false, branches')
 
   (* Fonction auxiliaire pour vérifier si un arbre est inutile *)
   and est_inutile (arbre : 'a arbre) : bool =
     match arbre with
     | Noeud (false, []) -> true
     | _ -> false
 
   (* Fonction publique pour éliminer les branches inutiles d'un arbre *)
   let eliminer_branches_inutiles_publique (arbre : 'a arbre) : 'a arbre =
     eliminer_branches_inutiles arbre
 end
