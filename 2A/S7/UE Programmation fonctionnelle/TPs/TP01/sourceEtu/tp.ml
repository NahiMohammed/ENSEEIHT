...
let coeff_directeur (x1,y1) (x2,y2) = if x1 = x2 then
   failwith "La droite est verticale, le coefficient directeur est indéfini."
 else
   (y2 -. y1) /. (x2 -. x1);;
...
let ieme triplet i = 
match i with
  | 1 -> let (x, _, _) = triplet in x
  | 2 -> let (_, y, _) = triplet in y
  | 3 -> let (_, _, z) = triplet in z
  | _ -> failwith "L'indice doit être entre 1 et 3"
;;
...
let rec dragon (xa,ya) (xb,yb) n = 
  if n = 0 then
    dessine_segment (xa,ya) (xb,yb)
  else
    let xm = (xa + xb) / 2 + (yb - ya) / 2 in
    let ym = (ya + yb) / 2 + (xa - xb) / 2 in
    dragon (xa, ya) (xm, ym) (n - 1);
    dragon (xb, yb) (xm, ym) (n - 1)
  ;;
