...
let evalue x p =
  let rec aux x p acc =
    match p with 
    |[a0]->acc +. a0
    |an::q ->aux x q ((acc +. an)*.x)
    |_-> 0.
  in aux x (List.rev p) 0.
     (*let%test  _ = evalue 2.5 [2.5] = 2.5
let%test _ = evalue 2.0 [2.0; 3.0] = 8.0
let%test _ = evalue 1.0 [2.0; 5.5; -2.0] = 5.5
let%test _ = evalue 2.0 [4.5; 6.0; -3.5; -8.0] = -61.5*)
...
let retract p =
  let rec aux p acc =
    match p with
    |[]->acc
    |c::cq->let has_numbre_non_nul =(List.exists (fun x-> x<>0. ) cq) in 
        if has_numbre_non_nul then (aux cq (acc @ [c] )) else (if c=0. then acc else (acc @ [c]))
  in aux p []
      ...
      let%test _ = retract [2.;0.;0.;0.;0.;0.] = [2.1]
...
let plus  p1 p2 =
  let rec aux p1 p2 acc=
    ...
    |[],_->(acc @ p2)
    |_,[]->(acc @ p1)
    |c::cq ,a::aq ->(aux cq aq (acc @ [a+.c]))
  in retract (aux p1 p2 [])
...
let get_branche c arbre1=
  match arbre1 with 
    |Noeud (right,middle,left) -> begin
                                  match c with 
                                  |1->right
                                  |2->middle 
                                  |3->left
    ...
    |Lettre _       ->raise CodeNonValide
    |vide           ->raise CodeNonValide

let decoder code arbre =
  let rec decode_list code_list arbre1 =
    match code_list with 
    |[] ->if arbre1=arbre then [] else raise CodeNonValide
    |c::qc ->begin 
             match get_branche c arbre1 with 
            |sous_arbre ->begin 
                           match sous_arbre with                                  
                           | Lettre sym -> sym::(decode_list qc arbre )
                           | _   -> decode_list qc sous_arbre 
                           end 
    
            end    
  in 
  recompose_chaine(decode_list (decompose_int code) arbre)
...
let arbre_to_liste arbre =
  let rec arbre_to_list_helper tree code_acc =
    match tree with 
    |Lettre symbole -> [(symbole,recompose_int(List.rev (code_acc)))]
    |Noeud (right,middle,left) ->List.concat [(arbre_to_list_helper right (1:: code_acc));(arbre_to_list_helper middle (2:: code_acc));(arbre_to_list_helper left (3 :: code_acc))]
    |Vide -> []
  in
