...
struct
 ...
 let arbre1 =
  ...
  let get_branche e arbre =
    match arbre with 
    |Noeud branches -> List.assoc_opt e branches 
    |Lettre _       -> None
  ...
  let dechiffrer code_liste arbre =
    let rec dechiffrer_aux code arb acc =
      match code with 
      |[]    ->List.rev acc
      |c::qc -> begin
                match get_branche c arb with 
                |Some sous_arbre ->begin 
                                   match sous_arbre with                                  
                                  | Lettre sym -> dechiffrer_aux qc arbre (sym :: acc) 
                                  | _   -> dechiffrer_aux qc sous_arbre acc
                                  end 
                | None -> raise CodeNonValide
                end 
      in dechiffrer_aux code_liste arbre []
...
exception CodeNonValide
  ...
  let arbre_to_liste arbre =
    let rec arbre_to_list_helper tree code_acc =
      match tree with
      | Lettre symbole -> [(symbole, List.rev code_acc)]
      | Noeud branches ->
        List.concat (List.map (fun (sym, sous_arbre) -> arbre_to_list_helper sous_arbre (sym :: code_acc)) branches)
    in
    arbre_to_list_helper arbre []
  ...
  (*  Exception DonneeNonValide : si la donnéee ne peut pas être chiffrée avec l'arbre de chiffrement **)exception DonneeNonValide

let chiffrer liste_symbole arbre =
  let arbre_assoc = arbre_to_liste arbre in

  let rec chiffrer_helper liste_symboles acc =
    match liste_symboles with
    | [] -> List.rev acc
    | symbole :: reste_symboles ->
      match List.assoc_opt symbole arbre_assoc with
      | Some code -> chiffrer_helper reste_symboles ((List.rev code) @ acc)
      | None -> raise DonneeNonValide
  in

  chiffrer_helper liste_symbole []
  ...
  (*  fold :  TO DO
  val fold_arbre_chiffrement :
  ('a -> 'b -> 'a) -> ('f -> 'a) -> ('a, 'f) arbre_chiffrement -> 'a

  let rec fold_arbre_chiffrement node_fn leaf_fn tree =
  match tree with
  | Lettre x -> leaf_fn x
  | Noeud (children) ->
    let rec fold_children acc = function
      | [] -> acc
      | child :: rest ->
        let result = fold_arbre_chiffrement node_fn leaf_fn child in
        fold_children (node_fn result acc) rest
    in
    fold_children (node_fn x) children
**)
  ...
  (*  nb_symboles : ('b,'f) arbre_chiffrement : ('b,'f) arbre_chiffrement -> int
   ...
   (* Fonction pour traiter les feuilles (Lettre) et renvoyer un ensemble de symboles *)
let leaf_fn symboles_set lettre =
  match lettre with
  | Lettre f -> StringSet.add f symboles_set

(* Fonction pour compter le nombre de symboles distincts dans l'ensemble *)
let nb_symboles arbre_chiffrement =
  let symboles_set = fold_arbre_chiffrement leaf_fn StringSet.empty arbre_chiffrement in
  StringSet.cardinal symboles_set
**)
...
type code = DRCode.mot
  ...
  (* dechiffrer : code -> (symbole_code,symbole_donnee) arbre_chiffrement -> donnee
     ...
     Exception CodeNonValide : si le code ne peut pas être déchiffré avec l'arbre de chiffrement
   *)
  let dechiffrer cod arbre  =let cod_list=DRCode.decompose cod in
  let donne_list=Arbre.dechiffrer cod_list arbre in

  DRDonnee.recompose donne_list
   ...
   *)
   let chiffrer donne arbre = DRCode.recompose( Arbre.chiffrer  (DRDonnee.decompose donne ) arbre) 
