...
let%test "Test 1" =
  let suite1 = suite 1 [ 1 ] in
  let check_no_digit_greater_than_3 term =
    List.for_all (fun digit -> digit <= 3) term
  in
  List.for_all check_no_digit_greater_than_3 suite1

let%test "Test 2" =
  let conway_sequence = suite 5 [ 1 ] in
  let check_no_digit_greater_than_3 term =
    List.for_all (fun digit -> digit <= 2)  term
  in 
  List.for_all check_no_digit_greater_than_3 conway_sequence
