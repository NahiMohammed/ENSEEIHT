...
struct
     ...
     *)
     let allocsize (bs, tr) =
        (* La taille en mémoire physique dépend de la hauteur de l'arbre. *)
        height tr
    ...
    let read (bs, tr) addr = search tr (bits bs addr)

    (* Implémentation de write. *)
    let write (bs, tr) addr value =  (bs, update tr value (bits bs addr))
end
