...
let rec conforme abre=
  match abre with 
  |Vide -> true
  |Noeud(b,lst)->if ((List.length lst) <> E.base) then false else (not (List.exists (fun x ->false) (List.map conforme lst)))
...
let rec appartient liste arbre =
  if(not (conforme arbre)) then false 
  else  
  match liste,arbre with 
  |[],Noeud(b,lst)     ->b=true
  |c::cq,Noeud(b,lst)  -> (appartient cq (get c lst))  
  |_,Vide ->false
...
let rec ajouter liste arbre =
  match liste arbre with
  |[],Vide            ->Noeud (true ,(l_taille E.base Vide )) 
  |c::cq,Vide         ->ajouter cq (Noeud (true ,l_taille E.base Vide ))
  |[],Noeud (b,lb)    -> if (not b) then Noeud (true,lb)
  |c::cq,Noeud (b,lb) ->
