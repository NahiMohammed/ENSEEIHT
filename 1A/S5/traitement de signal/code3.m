...
h0=signal_final.*cos(2*pi*F0*t+phi0); 
h1=signal_final.*cos(2*pi*F1*t+phi1); 
L=length(h0)/Ns;
S0= reshape(h0,Ns,L);
integ0=sum(S0);  
S1= reshape(h1,Ns,L);
...
%calcul taux d'erreur dans le  Contexte de synchronisation idéale
energie11=z>0;   
nbr_bits_err=zeros(1,L);
nbr_bits_err(find(energie11))=1;
erreurs= length(find(nbr_bits_err~=bits)); 
Teb_V21_synchro_ideale = erreurs/L   %taux d'erreur binaire obtenu
