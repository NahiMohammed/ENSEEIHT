...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, SearchBase)
...
# inputs
def depart_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, SearchBase)
        agent_object.departI = value
        # add code here if needed
    except:
        print(traceback.format_exc())

def arrivee_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, SearchBase)
        agent_object.arriveeI = value
        # add code here if 
        print("arrivee : " + value + "\n")
        searching(agent_object)
    except:
        print(traceback.format_exc())

import csv

def searching(agent_object):
    if agent_object.departI and agent_object.arriveeI:
        print(f"Recherche en cours...\nDépart : {agent_object.departI}\nArrivée : {agent_object.arriveeI}\n")

        def search_in_file(filename):
            results = []
            with open(filename, 'r') as file:
                reader = csv.reader(file, delimiter=',')
                results = [row for row in reader if row[:2] == [agent_object.departI, agent_object.arriveeI]]
            return results

        resAvion = search_in_file('avion.csv')
        resTrain = search_in_file('train.csv')
        resBus = search_in_file('bus.csv')

        print(f"Avion : {resAvion}\nTrain : {resTrain}\nBus : {resBus}\n")

        igs.output_set_string("resultTravelAvion", str(resAvion))
        igs.output_set_string("resultTravelTrain", str(resTrain))
        igs.output_set_string("resultTravelBus", str(resBus))
...
if __name__ == "__main__":
    ...
    igs.input_create("depart", igs.STRING_T, None)
    igs.input_create("arrivee", igs.STRING_T, None)

    igs.output_create("resultTravelAvion", igs.STRING_T, None)
    igs.output_create("resultTravelTrain", igs.STRING_T, None)
    igs.output_create("resultTravelBus", igs.STRING_T, None)

    igs.observe_input("depart", depart_input_callback, agent)
    igs.observe_input("arrivee", arrivee_input_callback, agent)
