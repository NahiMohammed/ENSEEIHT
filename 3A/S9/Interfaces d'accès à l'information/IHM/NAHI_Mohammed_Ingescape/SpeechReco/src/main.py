...
def speech_recognition():
    ...
    with sr.Microphone() as source:
        ...
        try:
            # Enregistrement de la voix avec un délai maximal de 5 secondes
...
# inputs
def Parole_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, SpeechReco)
        # add code here if needed
        igs.service_call("Whiteboard", "chat", " Veuillez parler pour indiquer votre souhait \n", "")
        demande = speech_recognition()
        while(demande==None):
            demande = speech_recognition()

        igs.output_set_string("reco", demande)
    except:
        print(traceback.format_exc())

def Ecoute_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, SpeechReco)
        agent_object.EcouteI = value
        # add code here if needed
        if(value == True):
            igs.service_call("Whiteboard", "chat", " Traitement de la demande terminée, veuillez parler pour effectuer une nouvelle demande \n" , "")
            demande = speech_recognition()
            while (demande == None):
                demande = speech_recognition()
            igs.output_set_string("reco", demande)
        else:
            igs.service_call("Whiteboard", "chat", " Traitement de la demande invalide, veuillez parler pour effectuer une nouvelle demande \n" , "")
            demande = speech_recognition()
            while (demande == None):
                demande = speech_recognition()
            igs.output_set_string("reco", demande)
...
if __name__ == "__main__":
    ...
    igs.input_create("Parole", igs.IMPULSION_T, None)
    igs.input_create("Ecoute", igs.BOOL_T, None)

    igs.output_create("reco", igs.STRING_T, None)

    igs.observe_input("Parole", Parole_input_callback, agent)
    igs.observe_input("Ecoute", Ecoute_input_callback, agent)
