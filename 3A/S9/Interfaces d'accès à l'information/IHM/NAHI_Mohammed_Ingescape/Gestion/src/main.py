...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, Gestion)
...
# inputs
def resultTravelAvion_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Gestion)
        agent_object.resultTravelAvionI = value
        # add code here if needed
    except:
        print(traceback.format_exc())

def resultTravelTrain_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Gestion)
        agent_object.resultTravelTrainI = value
        # add code here if needed
    except:
        print(traceback.format_exc())

def resultTravelBus_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Gestion)
        agent_object.resultTravelBusI = value
        # add code here if needed
        if(agent_object.resultTravelAvionI!=None and agent_object.resultTravelBusI!=None and agent_object.resultTravelTrainI!=None):
            choix(agent_object)
            agent_object.resultTravelAvionI=None
            agent_object.resultTravelBusI = None
            agent_object.resultTravelTrainI = None
            igs.output_set_bool("fini", True)
    except:
        print(traceback.format_exc())

def choix(agent):
    # Affiche un message indiquant que la recherche des possibilités est en cours
    igs.service_call("Whiteboard", "chat", "Recherche des possibilités en cours.\n\nFormat du résultat :\nVille de départ, ville d'arrivée, heure de départ, heure d'arrivée, prix", "")
    
    # Affiche les résultats pour chaque mode de transport
    igs.service_call("Whiteboard", "chat", f"Avion : {agent.resultTravelAvionI}", "")
    igs.service_call("Whiteboard", "chat", f"Train : {agent.resultTravelTrainI}", "")
    igs.service_call("Whiteboard", "chat", f"Bus : {agent.resultTravelBusI}", "")
...
if __name__ == "__main__":
    ...
    igs.input_create("resultTravelAvion", igs.STRING_T, None)
    igs.input_create("resultTravelTrain", igs.STRING_T, None)
    igs.input_create("resultTravelBus", igs.STRING_T, None)

    igs.output_create("result_voyage", igs.STRING_T, None)
    igs.output_create("fini", igs.BOOL_T, None)

    igs.observe_input("resultTravelAvion", resultTravelAvion_input_callback, agent)
    igs.observe_input("resultTravelTrain", resultTravelTrain_input_callback, agent)
    igs.observe_input("resultTravelBus", resultTravelBus_input_callback, agent)
